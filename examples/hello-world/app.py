...
configure_opentelemetry(
    HyperDXOptions(
        ...
        endpoint=os.getenv(
            "HYPERDX_API_ENDPOINT", None
        ),  # set specific endpoint if not HyperDX default
        endpoint_insecure=os.getenv(
            "OTEL_EXPORTER_OTLP_INSECURE", None
        ),  # default is False; set to True if setting insecure endpoint
...
def hello_world():
    token = attach(baggage.set_baggage("baggy", "important_value"))
    with tracer.start_as_current_span(name="hello"):
        token_second = attach(
            baggage.set_baggage("for_the_children", "another_important_value")
        )
